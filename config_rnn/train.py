    ...
    with tf.control_dependencies(update_ops_gpu0):
        if hasattr(config, 'optimizer') and config.optimizer == 'rmsprop':
            ...
            train_step = tf.train.AdamOptimizer(
                learning_rate=tf_lr, beta1=0.95,
                beta2=0.9995).apply_gradients(grads_and_vars=grads_and_vars,
                                              global_step=None,
                                              name='adam')
...
with tf.Session() as sess:
    ...
    for iteration, x_batch in zip(batch_idxs, train_data_iter.generate()):
        ...
        if iteration == 0:
            ...
            # Couldn't get past, got error about gpu
            ...
            if hasattr(config, 'validate_every') and (
                    iteration + 1) % config.validate_every == 0:
                print('\n Validating ...')
                losses = []
                rng = np.random.RandomState(42)
                for _, x_valid_batch in zip(
                        range(0, config.n_valid_batches),
                ...
                rng = np.random.RandomState(42)
                for _, x_valid_batch in zip(
                        range(0, config.n_valid_batches * 10),
