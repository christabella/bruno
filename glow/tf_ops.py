...
def actnorm_center(name, x, reverse=False):
    ...
    with tf.variable_scope(name):
        ...
        if len(shape) == 2:
            ...
            b = get_variable_ddi("b", (1, int_shape(x)[1]),
                                 initial_value=-x_mean)
        elif len(shape) == 4:
            x_mean = tf.reduce_mean(x, [0, 1, 2], keepdims=True)
            b = get_variable_ddi("b", (1, 1, 1, int_shape(x)[3]),
                                 initial_value=-x_mean)
...
def conv2d(name,
    ...
    with tf.variable_scope(name):
        ...
        if context1d != None:
            x += tf.reshape(linear("context", context1d, width),
                            [-1, 1, 1, width])
    return x
