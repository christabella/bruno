...
class FlowLayer:
    ...
    def __call__(self,
        ...
        with tf.variable_scope(scope_name), tf.variable_scope(
            ...
            if forward:
                ...
                print_shapes(f"{type(self).__name__}/Forward/{scope_name}",
                             True, outputs)
                ...
                outputs = identity_flow(outputs, forward=forward)
                print_shapes(f"{type(self).__name__}/Backward/{scope_name}",
                             False, outputs)
...
class QuantizeImage(FlowLayer):
    ...
    * After this Layer the image is mapped from [0, 1] to [-0.5, 0.5] range.
    """
    def __init__(self, num_bits: int = 8, **kwargs):
        """
...
class ActnormBiasLayer(_ActnormBaseLayer):
    ...
    def get_ddi_init_ops(self, num_init_iterations: int = 0) -> tf.Operation:
        ...
        bias_assign_op = tf.assign_add(self._bias_t,
                                       new_bias,
                                       name="bias_assign")
    ...
    def build(self):
        ...
        self._bias_t = tf.get_variable("bias",
                                       self.variable_shape,
                                       tf.float32,
...
class ActnormScaleLayer(_ActnormBaseLayer):
    ...
    def get_ddi_init_ops(self, num_init_iterations: int = 0) -> tf.Operation:
        ...
        log_scale_assign_op = tf.assign(self._log_scale_t,
                                        new_scale,
                                        name="log_scale_assign")
...
class ActnormLayer(FlowLayer):
    def __init__(
        ...
        self._scale_layer = ActnormScaleLayer(input_shape=input_shape,
                                              scale=scale)
...
class InvertibleConv1x1Layer(FlowLayer):
    ...
    def build(self):
        ...
        if not self._use_lu_decomposition:
            ...
            dlogdet = (tf.cast(
                tf.log(tf.abs(tf.matrix_determinant(tf.cast(w, dtype)))),
...
class AffineCouplingLayer(FlowLayer):
    def __init__(
            ...
            name: str = "",
            log_scale_fn: Callable[[tf.Tensor], tf.Tensor] = lambda x: tf.exp(
                tf.clip_by_value(x, -15.0, 15.0)),
            **kwargs,
    ):
        """
